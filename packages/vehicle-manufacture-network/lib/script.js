...
function setupDemo() {
 	console.log('setupDemo');
  	...
  	var manufacturers; // will use those found in vehicles object
    ...
    people = people.map(function (person) {
  	...
  	// create array of Manufacturer particpant resources identified by the top level keys in vehicles var
  	manufacturers = Object.keys(vehicles).map(function (manufacturer) {
    	var manufacturerResource = factory.newResource(namespace, 'Manufacturer', manufacturer);
      	manufacturerResource.name = manufacturer;
      	return manufacturerResource;
    });
  	
  	// create a Regulator participant resource
    var regulator = factory.newResource(namespace, 'Regulator', 'VDA');
  	regulator.name = 'VDA';
  	
  	// add the regulator
  	return getParticipantRegistry(namespace + '.Regulator')
    .then(function (regulatorRegistry) {
      return regulatorRegistry.add(regulator);
    })
    .then(function () {
      return getParticipantRegistry(namespace + '.Manufacturer');
    })
      ...
      return personRegistry.addAll(people);
    })
	.then(function () {
      return getAssetRegistry(namespace + '.Vehicle');
    })
      ...
      for (var manufacturer in vehicles) {
          for (var model in vehicles[manufacturer]) {
              var vehicleTemplatesForModel = vehicles[manufacturer][model];
              vehicleTemplatesForModel.forEach(function(vehicleTemplate) {
                  var vehicle = factory.newResource(namespace, 'Vehicle', vehicleTemplate.vin);
      ...
      return vehicleRegistry.addAll(vehicleResources);
    });
}